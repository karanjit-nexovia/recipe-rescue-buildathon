...
**Drop in a reel** — the audio is transcribed and on-screen text is read from the video
frames, which is often where the exact quantities actually live. No video? Paste the
caption instead.
...
Two pipelines. `transcribe.pipe` turns media into text — `audio_transcribe` for speech,
`frame_grabber` into `ocr` for on-screen text. It runs no model, so it costs nothing.
